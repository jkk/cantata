...
(defn reflect-fields [ds table]
 ...
 (with-open [conn ^Connection (jd/get-connection ds)]
   (let [dbmeta ^DatabaseMetaData (.getMetaData conn)
         ...
         cmetas (if (seq cmetas) ;H2 quirk
                  cmetas
                  (doall
                    (jd/result-set-seq
                      (.getColumns dbmeta nil nil (string/upper-case table) nil))))]
     (doall
       (for [cmeta cmetas]
...
(defn reflect-rels [ds table]
  ...
  (with-open [conn ^Connection (jd/get-connection ds)]
    (let [dbmeta ^DatabaseMetaData (.getMetaData conn)
          ...
          imetas (if (seq imetas) ;H2 quirk
                   imetas
                   (jd/result-set-seq
                     (.getImportedKeys dbmeta nil nil (string/upper-case table))))]
      (doall
        (for [imeta imetas]
 ...
 (with-open [conn ^Connection (jd/get-connection ds)]
   (let [dbmeta ^DatabaseMetaData (.getMetaData conn)
         ...
         imetas (if (seq imetas) ;H2 quirk
                  imetas
                  (jd/result-set-seq
                    (.getPrimaryKeys dbmeta nil nil (string/upper-case table))))
